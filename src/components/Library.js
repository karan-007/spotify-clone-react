...
function Library({ history }) {
    ...
    function handleClick(pid) {
        ...
        const newPlaylist = playlist.filter(playlist => playlist.playlist_id !== pid);
    ...
    if (toggle) {
        playListData =
            <div className="playlist-cards">
                <h1 className='mt-4'>Your Library</h1>
                <div className="cardlist">
                    {playlist.map((list) => {
                        return <Card
                            onClick={() => goto(`/playlist/${list.playlist_id}/${list.playlist_name}`)}
